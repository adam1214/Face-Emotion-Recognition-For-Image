# -*- coding: utf-8 -*-
"""
Created on Tus Sep 22 14:58:40 2020
...
def set_up():
    ''' Move test image to feri_input '''
    copytree('feri_verification/input', 'feri_input')
    files = folders = 0
    for _, dirnames, filenames in os.walk('feri_verification/input'):
        files += len(filenames)
...
def test_mode_1(set_up):
    ...
    for root, dirs, files in os.walk('feri_verification/result/folder1'): 
        for file_ in files:
            veri_df = pd.read_csv('feri_verification/result/folder1/' + file_)
            test_df = pd.read_csv('feri_result/folder1/' + file_)
            print('feri_result/folder1/' + file_)
...
def test_mode_2(set_up):
    ...
    print(bools)
    j = 0
    for root, dirs, files in os.walk('feri_verification/result/'):
        for file_ in files:
            test_path = os.path.join(root, file_)[24:]
            test_path = "feri_result" + test_path
